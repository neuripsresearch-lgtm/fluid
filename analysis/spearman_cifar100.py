...
HAF_TREE_FILE = './assets/tree_haf.json'
CLASSES_FILE = './assets/cifar100_classes.txt'
...
def build_tree_maps(tree: dict) -> tuple[dict, dict, dict]:
    ...
    using unique IDs to avoid name collision issues (e.g., 'worm' parent/child).
    Returns:
        parent_map (dict): {child_unique_key: parent_unique_key}
        depth_map (dict): {unique_key: depth}
        name_to_key_map (dict): {leaf_name: unique_key}
    """
    ...
    def dfs(node, parent_key, depth):
        ...
        if children:
            ...
            if name in name_to_key_map:
                # This case shouldn't happen with CIFAR100
                print(f"Warning: Duplicate leaf node name found: {name}. Overwriting in map.")
...
def print_results(corr_my_spearman, corr_haf_spearman, corr_my_pearson, corr_haf_pearson):
    ...
    print(f"HAF Tree (tree_haf.json):   ρ = {score_haf_s:.6f} (p-value: {corr_haf_spearman.pvalue:.2e})")
