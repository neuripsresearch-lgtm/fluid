...
def main(args):
    ...
    # 1. Data Setup
    # EDITED: Changed Normalization to 0.5 (neutral) as FER2013 is grayscale-converted-to-RGB
    ...
    print("Loading FER2013 Dataset (Train Split)...")
    
    # EDITED: Use ImageFolder instead of CIFAR100
    # Assumes structure: args.data_path/train/class_name/image.jpg
    train_dir = os.path.join(args.data_path, 'train')
    if not os.path.exists(train_dir):
        raise FileNotFoundError(f"Train data not found at {train_dir}. Please structure data as folders.")

    full_trainset = torchvision.datasets.ImageFolder(root=train_dir, transform=transform_train)
    ...
    print(f"Splitting Data: {len(train_idx)} Train / {len(val_idx)} Validation")

    train_subset = Subset(full_trainset, train_idx)
    
    # EDITED: Reload validation set using ImageFolder with VAL transforms
    full_valset_raw = torchvision.datasets.ImageFolder(root=train_dir, transform=transform_val)
    ...
    # Note: num_classes changed to len(full_trainset.classes) which is 7
    model = timm.create_model('swin_tiny_patch4_window7_224', pretrained=True, num_classes=len(full_trainset.classes))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hierarchy-path', default='./assets/fer2013/curr_tree.json', type=str)
    parser.add_argument('--save-path', default='./weights/mbm_model_fer2013.pth', type=str)
