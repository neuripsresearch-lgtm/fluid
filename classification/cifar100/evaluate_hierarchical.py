...
def main(args):
    ...
    # --- SPLIT LOGIC (Validation vs Test) ---
    if args.split == 'test':
        print("LOADING: OFFICIAL TEST SET (Evaluating Final Model)")
        dataset = torchvision.datasets.CIFAR100(root=args.data_path, train=False, download=True, transform=transform)
    else:
        print("LOADING: VALIDATION SPLIT (Optimizing Tree/Model)")
        # Load train set
        full_trainset = torchvision.datasets.CIFAR100(root=args.data_path, train=True, download=True, transform=transform)
        # Recreate the exact split from training script
        num_train = len(full_trainset)
        indices = list(range(num_train))
        split = int(np.floor(0.1 * num_train)) # 10%
        
        # Shuffle deterministically (Seed 42)
        np.random.shuffle(indices)
        
        # Take Validation portion (first 10% in this shuffling logic)
        val_idx = indices[:split]
        dataset = Subset(full_trainset, val_idx)
        print(f"Validation subset size: {len(dataset)}")

    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=4)
    
    # Classes are always consistent in CIFAR100
    # To be safe, we instantiate a dummy raw dataset to get class list if using Subset
    raw_cifar = torchvision.datasets.CIFAR100(root=args.data_path, train=False, download=False)
    classes = [c.replace(' ', '_') for c in raw_cifar.classes]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--misclass-threshold', default=5, type=int)
