...
def get_llm_response(prompt, max_tokens=32000, temperature=0.2, model="gemini-2.5-pro", # Changed default model to gemini
...
def edit_tree(current_tree, edit_instructions, prompt_file_path, classes_file_path, model="gemini-2.5-pro", max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        response_text = get_llm_response(full_prompt, max_tokens=16000, model=model,
